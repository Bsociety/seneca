...
exports.patternWithout = function patternWithout(patobj, fields) {
  var pattern = pattern(patobj)
  fields = _.isString(fields) && [fields] || fields
  fields = _.isArray(fields) && fields.map(field => `${field}:`) || []
  return pattern.split(',')
    .filter(value => value.indexOf(fields) === -1)
    .join(',')
}
...
exports.makedie = function(instance, ctxt) {
  ...
  var die = function(err) {
    ...
    var full = (so.debug && so.debug.print && ('full' === so.debug.print.fatal))
        || false
    ...
    try {
      ...
      //var explain = 'Action called with fatal$:true. See http://senecajs.org/api/#modifier-fatal'

      //var pat = Jsonic(err.details.pattern)
      //console.log(err)
      /*
      if( 'seneca' === pat.role ) {
        if( 'define' === pat.plugin ) {
          explain = error('explain_plugin_define', {
            plugin: pat.name,
      ...
      */
      ...
      var stderrmsg =
        ...
        '\nDETAILS   ::: ' +
          (Util.inspect((
            full ? err.details :
              _.omit(clean(err.details),['instance'])), { depth: null }))
           .replace(/\n/g, '\n              ') +
        '\nSTACK     :::  ' +
        stack +
